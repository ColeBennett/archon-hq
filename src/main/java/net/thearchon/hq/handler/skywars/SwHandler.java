...
public class SwHandler extends GameHandler<SwClient> {

    private final File schemDir;
    private final Random random = new Random();
    
    public SwHandler(Archon archon) {
    ...
    public void requestReceived(Client client, final RequestPacket request, Protocol header, BufferedPacket buf) {
        if (header == Protocol.GAME_REQUEST_JOIN) {
            ...
            if (buf.hasIndex(1)) {
                SwClient cl = getClient(buf.getInt(1));
    ...
    @Override
    protected void handleInbound(SwClient client, Protocol header, BufferedPacket buf) {
        if (header == Protocol.GAME_WIN) {
            Player player = server.getPlayer(buf.getInt(0));
            if (player != null) {
                SwLobbyHandler handler = server.getHandler(ServerType.SKYWARS_LOBBY);
    ...
    @Override
    protected void handleConnect(SwClient client) {
    ...
    @Override
    protected void handleDisconnect(SwClient client) {
